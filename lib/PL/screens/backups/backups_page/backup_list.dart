...
class BackupList extends StatelessWidget {
  ...
  const BackupList({Key? key,this.backupProvider}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: backupProvider!.backupsStream(),
    ...
    snapshot.data!.size==0
        ?
    const NoDataCard(message: "لا يوجد backups",)
        :
    ...
    ListView.builder(
        ...
        itemBuilder: (context,position){
          ...
          BackupModel backupModel=BackupModel.fromSnapshot(snapshot.data!.docs[position].data());
