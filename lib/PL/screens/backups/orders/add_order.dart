...
class AddOrder extends StatelessWidget {
  ...
  String? orderId;
   ...
   double? oldPrice;
   final String? type;
  AddOrder({Key? key,this.backupId,this.type,this.backupDate}) : super(key: key);
  AddOrder.update({Key? key,this.backupId,this.orderId,this.type,
    this.customerName,this.phone,this.phone2,this.chargeValue,this.price,this.address,this.backupDate}) : super(key: key);
  ...
  Map<String,dynamic> setData(){
    ...
    OrderRef.supplierStatus:OrderRef.waiting,
    OrderRef.providerStatus:OrderRef.waiting,
  ...
  ElevatedButton _addButton(OrdersProvider ordersProvider,BuildContext context) {
      return
        ElevatedButton(
            onPressed: (){
                ...
                if(_formKey.currentState!.validate())
                {
                  if(type=="إضافة طلب")
                  {
                    ordersProvider.addOrder(data: setData(),backupDate: backupDate,context: context);
                  }
                  else
                  {
                    ordersProvider.updateOrder(data: setData(),oldPrice:oldPrice,backupDate: backupDate,context: context );
                  }

                }

           },
            child: SizedBox(width:double.infinity,height: 50,child: Center(child: Text(type!))));
