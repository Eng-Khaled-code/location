...
class PhoneWidget extends StatelessWidget {
  ...
  Row _addPhoneRow(BuildContext context) {
    return Row(
      children: [
        const Text("ارقام التليفون",),
        const SizedBox(width :15),
        IconButton(icon:const Icon(Icons.add_ic_call_rounded),onPressed: ()=>
            showDialog(context: context, builder: (context)=>ProfileCustomDialog(type: "phone",addOrUpdateOrDelete: "add",fieldValue: "",fieldId: "")
              ,))
      ],
    );
  }
  ...
  Container _phoneList(BuildContext context) {
    Stream myStream=FirebaseFirestore.instance.collection(PhoneRef.phoneCollectionRef)
        .where(PhoneRef.userId,isEqualTo: userModel!.userId).get().asStream();
    return Container(
  ...
  InkWell _phoneCard(PhoneModel phoneModel, BuildContext context,int length) {
    ...
    return InkWell(
    onTap: ()=>
        showDialog(context: context, builder: (context)=>ProfileCustomDialog(type: "phone",addOrUpdateOrDelete: "update",fieldValue: phoneModel.number,fieldId:phoneModel.phoneId)),
    child: Container(
        ...
        children: [
          GestureDetector(child:const Icon(Icons.close,color: Colors.red,),
            onTap: (){
              ...
              {
                showDialog(context: context, builder: (context)=>ProfileCustomDialog(type: "phone",
                    addOrUpdateOrDelete: "delete",
                    fieldValue: phoneModel.number,
                    fieldId: phoneModel.phoneId));
              }},),

          Text(phoneModel.number!,),
        ],
      ),),
  );
  }
